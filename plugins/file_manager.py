...
def makedir_ls(client: Client, callback_query: CallbackQuery):
    with Session(engine) as session:
        ...
        sub_directories = session.scalars(select(Directory).where(Directory.id == int(directory_id))).all()
        ...
        if len(directory_path := callback_query.data.split('/')) > 1:
            keyboard.append([InlineKeyboardButton("بازگشت به فولدر قبلی", callback_data=f"{directory_path[:-1]}")])
...
def naming_made_directory(client: Client, message: Message):
    with Session(engine) as session:
        persian_title, title = message.split('-')
        ...
        if user_cmd.split('-')[-1] == 'root':
            ...
            parent_directory = user_cmd.split('-')[-1].split('/')[-1]
